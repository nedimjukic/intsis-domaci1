...
class Aki(Agent):
    def __init__(self, x, y, file_name):
        super(Aki, self).__init__(x, y, file_name)
    ...
    def get_agent_path(self, coin_distance):
        ...
        while len(path) < len(coin_distance):
            ...
            minimum = sys.maxsize
            for i in range(0, len(coin_distance[curr_index])):
                if i == curr_index:
                    continue
                if minimum > coin_distance[curr_index][i] and (i not in path):
                    minimum = coin_distance[curr_index][i]
                    next_index = i
